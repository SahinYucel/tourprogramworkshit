...
const TourAddToList = () => {
  ...
  // Verileri yüklemek için yeni useEffect
  useEffect(() => {
    const fetchTourData = async () => {
      try {
        ...
        if (data.tours) {
          setTours(data.tours);
          setCounter(Math.max(...data.tours.map(t => t.id), 0) + 1);
        }
        
        if (data.bolgeler) {
          setBolgeler(data.bolgeler);
          setBolgeCounter(Math.max(...data.bolgeler.map(b => b.id), 0) + 1);
        }
        ...
        if (data.regions) {
          ...
          setRegionCounter(Math.max(...data.regions.map(r => r.id), 0) + 1);
        }
      } catch (error) {
